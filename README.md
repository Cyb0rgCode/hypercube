...
- **GitHub:** [`Cyb0rgCode/Dash`](https://github.com/Cyb0rgCode/Dash) — the
  ...
  if you want to extend this system: every visual choice here is rooted in
  that codebase.
...
| **Matrix** | Eisenhower 2×2 grid of tasks by urgency × importance. |
| **Tasks** | Create, prioritize, batch-edit, filter, and time-log tasks. Grouped by category → chapter. |
...
- Greeting (time-aware): `Burning the midnight oil` · `Good morning` · `Good afternoon` · `Good evening` · `Late night focus`
- Subtitle when no tasks pending: `Inbox zero — nice`
- Focus banner with no tasks: `All clear — add a task to plan your day`
- Focus banner meta with 1 task: `Last one standing`
- Pending filter empty state: `Inbox zero` / `All caught up — no pending tasks.`
- Tasks meta: `3 pending · 2 done today`
- Toasts: `Task added` (success), short and present-tense.

**Tags / chips.** Lowercase, mono, often with a single leading glyph that
substitutes for words: `! urgent`, `★ key`, `⏱ 1h 20m`, `~45m` (estimated),
`⚙ medium`. The glyph is the brand — never strip it.
...
(`Inbox zero — nice`, `All clear — add a task`). Not hyphens.

**No emoji** in chrome. Single Unicode glyphs do appear sparingly as icons
in dense rows (`✓` for done, `✕` for delete, `✎` for edit, `⏱` for time
logged, `!` and `★` in tags). Treat these as iconography, not as decoration.
...
no patterns, no decorative imagery — the brand is the gradient.
...
- Muted slate-violet `#6060a0` is the workhorse for meta, labels, dividers.
  ...
  Every semantic color has a `-dim` variant at ~13–15% alpha, used as the
...
for moments of emphasis — never as a section background.

**Typography.** Two families do everything.
- **Syne** (700/800) — every heading, every button label, every task title.
...
Items in lists are `8px` apart, fields in forms `10px–12px`. Generous outer
margins (`36–40px`) on desktop content; tight on mobile.
...
larger soft-square radii (`11–14px`) rather than full circles.
...
left-border priority coding on task rows — a 3px accent border in pink
(urgent+important), mint (important), or amber (urgent). Matrix quadrants
get a 3px top accent + faint matching wash.

**Hover.** Soft. Most controls just lighten color (muted → text, or
muted → accent) and tint background with the matching `-dim` color. Cards
...
(`0.4s`). This is the iOS-feel — applied to every button across the app.

**Animation.** Three named curves, used everywhere:
- `--spring` `cubic-bezier(0.34, 1.56, 0.64, 1)` — taps, sliding nav
  indicators, segmented control thumbs, card entrances.
- `--ease-out` `cubic-bezier(0.22, 1, 0.36, 1)` — modal open, page
  transitions, hover state changes.
- `0.18s` `ease` — color/border crossfades on hover.

Entrance: `translateY(14px) scale(0.98) → 0,1` over `0.5s`, staggered
60ms per card. Tab switches: `translateY(6px) → 0` over `0.18s`. The
focus-banner radial glow breathes in a `7s` loop at `scale(1.18)`.

**Transparency & blur.** Three glass surfaces:
1. Sidebar — `rgba(18,18,26,0.75)` + `blur(20px)`.
2. Sticky category headers in lists — `rgba(10,10,15,0.78)` + `blur(18px)
   saturate(180%)`.
3. Mobile floating nav dock — `rgba(18,18,26,0.62)` + `blur(28px)
   saturate(180%)`.

No blur is ever applied to body content. Modals use a flat 55% black
backdrop + 4px blur — not as glassy as the chrome.
...
- `viewBox="0 0 24 24"` with `width:height` of 14–24px depending on context.
- `fill: none; stroke: currentColor` — icons inherit color from text.
...
- A handful of solid fills appear inside otherwise stroked icons — e.g.
  the bullseye dot in the habits / focus icon (`<circle fill="currentColor">`).

The set is small and purposeful — bar chart, 2×2 grid, checklist, target,
checkmark, flame, in-tray, copy, upload — drawn to match
...
be too heavy: `✓` `✕` `✎` `⏱` `★` `!`. These are styled with the same color
tokens as the SVGs.
...
- `assets/logo-mark.svg` — the 4-tile stacked grid mark used in the sidebar
  and as the app icon. Renders inside a `--hk-gradient-brand`-filled rounded
  square.
- `assets/icons/*.svg` — extracted nav + stat icons (`dashboard`, `matrix`,
...
- For anything outside this set, use [Lucide](https://lucide.dev/) at
  `stroke-width: 2`, `round` joins/caps — it's the closest match.

## License

Personal project, no explicit license in the source repo.
